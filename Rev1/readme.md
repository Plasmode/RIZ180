...
### Software
- [RIZMon](Software/rizmon_v0_6.zip) is a simple monitor for RIZ180. The serial port setting for 18.432MHz clock is 57600 N81
- [CP/M2.2 for RIZ180](Software/cpm22all_riz180_12_19_20.zip). To install it, send cpm22all.hex to RIZ180 and type 'c2' to install it in track 0 of CF disk. Once it is installed, type 'b2' to load and run CP/M2.2
...
- [SCMonitor](Software/riz180_scmon.zip) patched for RIZ180. This hex file should be burn into EPROM replacing RIZMon. This is SCMonitor for SC126 patched to run on RIZ180. The difference between SC126 and RIZ180 is the base address of RAM; SC126's RAM is located from 0x80000 to 0xFFFFF whereas RIZ180 RAM is from 0x40000 to 0x5FFFF. By patching the MMU initialization routine at location 0x1D3F where common block 1 is assigned, SCMonitor for SC126 will now run on RIZ180.
- [SCMonitor as an application for RIZ180](Software/scmon_loadable.zip). This hex file is loaded into RIZ180's memory and saved to CF disk as an application using 'c1' command. Afterward it can be recalled into memory and execute by using 'b1' command. This is patched SCMonitor for SC126 where the MMU initialization routine is replaced with NOP instructions.
- [SCMonitor + Startrek as an application for RIZ180](Software/loadable_riz180_scmon_startrek.zip). This hex file is similar to the file one line above, except the BASIC program has 48K of usable memory and is pre-loaded with StarTrek. To run the pre-loaded StarTrek, warm start BASIC with 'wbasic', then 'run'.
- [CPM3ALL](https://github.com/Plasmode/Z80SBC64/blob/main/Software/cpm3all.zip) . This is the CP/M 3 distribution files. XMODEM cpm3all.arj to drive A of CF disk and use unarj.com to decompress it.
- [CP/M3 Loader](Software/cpmldr_for_riz180.zip). This is loader for CP/M3. It expects CPM3.SYS in drive A of the CF disk. First install CPM3ALL above in drive A of the CF disk. To install CP/M3 loader, send CPM3LDR.HEX to RIZ180 and type 'c3' to install it in track 0 of CF disk. Once it is installed, type 'b3' to boot CP/M3.
- [CPM3 banked BIOS source code](Software/riz180_cpm3_cbios3_ldrbios_sources.zip), assembled with zmac
